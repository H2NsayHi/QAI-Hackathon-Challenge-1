...
class Preprocess:
    def __init__(
        ...
        if self.find_best_rotation:
            self.crop_path = os.path.join(CACHE_DIR, 'image_crops')
...
class Detection:
    ...
    def __call__(
        ...
        if output_path is None:
            ...
            output_path = os.path.join(output_path, 'image_crops')
...
class OCR:
    def __init__(self, config_path=None, weight_path=None, model_name=None):
        ...
        config = Config(config_path)
        ocr_config = ocr.Config.load_config_from_file("tool/config/ocr/models/vgg-seq2seq.yml")
